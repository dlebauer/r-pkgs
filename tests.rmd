...
It is often useful to use small data samples in testing. While it often may be possible to use internal datasets such as `data(iris)` or `letters` in testing, sometimes it is useful to use your own example data. Test data that isn't used elsewhere in the package can be placed in the `/tests/testthat/data` and accessed from within a test using the relative path starting with 'data', for example, if the files `mytestdata.csv` is in this directory, the file can be accessed within a testing script thus:

```r
test_that("myfunction works with mytestdata", {
  mytestdata <- read.csv('data/mytestdata.csv')
  mytestoutput <- read.csv('data/mytestoutput.csv')
  myfunctionoutput <- myfunction(mytestdata)
  expect_equal(myfunctionoutput, mytestoutput)
})
```
